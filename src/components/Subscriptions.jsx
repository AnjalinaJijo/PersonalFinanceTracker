...
import {
  ...
  selectTriggered,
  setTriggered,
} from "@/lib/features/subscription/subscriptionSlice";
import {
  setCategoryItems,
  selectCategoryItems,
...
export default function Subscriptions() {
  ...
  //  // Check if subscriptionArray is available
  //  if (!subData) {
  //   return <div>Loading...</div>; // Or render an appropriate loading indicator
  // }
  ...
  const renderCell = useCallback((sub, columnKey) => {
    ...
    switch (columnKey) {
      case "name":
        return <div>{sub.subscriptionName}</div>;
      case "amount":
        return <div>{sub.subscriptionAmount}</div>;
      case "status":
        ...
        return (
          <div className="relative flex items-center gap-2">
            <Tooltip content="Edit sub">
              <span
                onClick={() => handleEdit(sub.subscriptionID, sub)}
  ...
  const handleSaveSub = async () => {
    const body = {
      ...
      subscriptionAmount: newSubscription.amount,
      status: "Unpaid",
    };
    ...
    <div>
      {editColumns.map((column) => (
        <div key={column.uid} className="m-2">
          {column.uid === "category" ? (
            <Dropdown showArrow radius="md">
              <DropdownTrigger>
                <Button size="md" className="text-left">
                  {newSub[column.uid]}
                </Button>
              </DropdownTrigger>
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      const body = {
        subscriptionName: editedValue["name"],
        subscriptionAmount: editedValue["amount"],
        Date: formattedDate,
      ...
      <div>
        ...
        <div>
          <Tooltip color="danger" content="Delete sub">
            <span
              onClick={() => handleDelete(editedValue.subscriptionID)}
              className="text-lg text-danger cursor-pointer active:opacity-50"
            >
              <DeleteIcon />
            </span>
  ...
  return (
    <div className="w-full">
      ...
      {!addNewClicked && !editedValue && (
        ...
        >
          ...
          <TableBody
            ...
            items={addNewClicked ? [newSubscription] : subData}
          >
            {(item) => (
      ...
      {addNewClicked && renderAddNew(newSubscription, editColumns)}

      {editedValue && renderEditing(editedValue, editColumns)}
    </div>
  );
}
